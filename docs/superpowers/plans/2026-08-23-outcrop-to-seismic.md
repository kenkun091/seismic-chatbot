...
- Deviations from the spec, decided while planning (each noted where it applies): `VisionClient.interpret_image` returns raw text and JSON parsing/retry lives in `outcrop_tools`; shale default `vclay` is 0.50 (Han's range ceiling — the spec's 0.70 would warn-and-clip on every run); `dt` default is 1.0 ms (not 0.001 s); the wedge oracle is covered transitively (the 1-D tool is already oracle-tested against the wedge), so Task 6 tests against the 1-D tool only; the recipe returns the section plot as `image_path` and the interpretation overlay in `extra_image_paths`; `create_synthetic_section`'s depth-domain return is `(z_axis, section, parameters)` — the 1-D-tool ordering (axis, data, parameters) — not the spec's `(section_depth, z_axis)`; there are two independent paddings (`outcrop_to_model`'s `pad_m`, added as background depth rows, and `create_synthetic_section`'s `pad_time`, quiet time in the seismic axis) rather than one, and `plot_seismic_section` crops the model/section panels to the outcrop extent ± one dominant wavelength when the model carries `image_top_m`/`height_m` (final-fix-brief #3), instead of always showing the full padded grid.
